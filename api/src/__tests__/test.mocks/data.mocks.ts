...
export const tasks = [
    ...
    { "id": "24", "description": "Write down your five best guesses about the future and send them to future You in five years using futureme.org" },
...
export const tasksOrder = tasks.reduce((acc, curr) => {
    acc[curr.id] = curr
    return acc;
    }, {}
);

export const tasksStatus = tasks.reduce((acc, curr, i) => {
    if (i === 0) {
        acc[curr.id] = {
            state: 'success',
...
export const achievementsStatus = achievements.reduce((acc, curr) => {
    acc[curr.id] = {
        state: 'pending',
